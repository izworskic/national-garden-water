...
import {computeDecision,gallonsFor,projectSevenDays,replayObservedWaterBalance} from '../public/assets/national-garden-water-engine.js';
...
test('unknown starting moisture is a range, never a guessed average',()=>{
  ...
  assert.equal(r.minDepletionIn,0);
  assert.equal(r.maxDepletionIn,2.88);
});
...
test('actual dry observed history can prove watering is needed',()=>{
  ...
  assert.equal(d.state,'WATER TODAY');
  assert.ok(d.metrics.minDepletionIn>=d.metrics.stressTriggerIn);
  assert.match(d.reason,/replaying .* observed days of NOAA/i);
});
...
test('actual soaking history can collapse uncertainty to a no-water result',()=>{
  ...
  assert.ok(d.metrics.maxDepletionIn<d.metrics.stressTriggerIn);
});
...
test('missing historical coverage does not invent a wet-enough answer',()=>{
  ...
  assert.equal(d.state,'CHECK SOIL FIRST');
  assert.match(d.reason,/cannot honestly choose/i);
});
...
test('logged irrigation reverses an otherwise-water decision using observed drying',()=>{
  ...
  assert.equal(a.state,'WATER TODAY');
  assert.notEqual(b.state,'WATER TODAY');
  assert.ok(b.metrics.minDepletionIn<a.metrics.minDepletionIn);
});

test('older irrigation gets less remaining credit based on observed ET',()=>{
  const fresh=computeDecision({...base,irrigationIn:1,irrigationAgeDays:0});
  const old=computeDecision({...base,irrigationIn:1,irrigationAgeDays:5});
...
test('current soil feel still overrides weather uncertainty',()=>{
  ...
  assert.equal(a.metrics.minDepletionIn,b.metrics.minDepletionIn);
  assert.equal(a.metrics.minDepletionIn,a.metrics.maxDepletionIn);
});

test('rain gauge corrects rather than double-counts recent observed rainfall',()=>{
  const a=computeDecision({...base,recentRainIn:.2,rainGaugeIn:1.2});
  const b=computeDecision({...base,recentRainIn:.2,rainGaugeIn:.2});
...
test('NCEI station selection rewards complete nearby daily records',()=>{const rows=[
  {STATION:'A',NAME:'Near sparse',LATITUDE:'43.60',LONGITUDE:'-83.90',DATE:'2026-09-01',PRCP:'0.1',TMAX:'80',TMIN:'60'},
  ...Array.from({length:5},(_,i)=>({STATION:'B',NAME:'Complete',LATITUDE:'43.70',LONGITUDE:'-83.80',DATE:`2026-09-0${i+1}`,PRCP:'0',TMAX:'78',TMIN:'58'}))
];const s=chooseNceiDailyStation(rows,43.6,-83.9);assert.equal(s.station,'B');assert.equal(s.completeDays,5)});
...
test('engine permanently bans the synthetic starting-reserve baseline',()=>{
  ...
  assert.doesNotMatch(js,/taw\s*\*\s*0\.42|conservative modeled starting reserve/);
  assert.match(js,/NOAA\/NWS observed-weather uncertainty range/);
  assert.match(js,/replayObservedWaterBalance/);
});
...
test('canonical page owns only garden-water assets and uses current module runtime',()=>{
...
test('default page stays location-first with advanced inputs collapsed',()=>{
  const html=fs.readFileSync(path.join(root,'public/national-tools/garden-water/index.html'),'utf8');
  assert.match(html,/id="location-form"/);assert.match(html,/Garden Water Today/);assert.match(html,/id="result" hidden/);assert.match(html,/<details class="adjustments" id="adjustments">/);assert.match(html,/id="rain-gauge"/);
});

test('page controller sends observed history into the decision',()=>{
  const js=fs.readFileSync(path.join(root,'public/assets/national-garden-water-page.js'),'utf8');
  assert.match(js,/observedHistoryDays:context\.observedHistory/);
  assert.match(js,/currentObservedRainIn:Number\(context\.recentRain\?\.todayIn/);
  assert.match(js,/NOAA observed history/);
  assert.match(js,/national-garden-water-engine\.js\?v=20260904-v4/);
  assert.doesNotMatch(js,/conservative modeled starting reserve/);
});

test('result states retain distinct visual emphasis',()=>{
  const css=fs.readFileSync(path.join(root,'public/assets/national-garden-water.css'),'utf8');
  assert.match(css,/data-state="wait"/);assert.match(css,/data-state="check-soil-first"/);assert.match(css,/data-state="hold-for-rain"/);assert.match(css,/data-state="water-today"/);
});
