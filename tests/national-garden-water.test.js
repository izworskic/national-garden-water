...
test('rain that ended in the current hour remains age zero and counts as recent',()=>{const d=computeDecision({...base,soilFeel:'unknown',recentRainIn:.75,recentRainAgeHours:0,recentRainConfidence:'medium'});assert.equal(d.metrics.recentRainAgeHours,0);assert.equal(d.metrics.soakingRecentRain,true);assert.equal(d.state,'WAIT')});
